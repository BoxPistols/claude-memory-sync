...
A. 問題ありません。プロジェクト固有の記憶は `repos/{git-remote-slug}.md` に分離され、注入先も `<project-root>/CLAUDE.local.md` とプロジェクトごとに分かれるので、何個でも並行可能です。global.md は全プロジェクトで共有され、`~/.claude/CLAUDE.md` へ注入されます。
